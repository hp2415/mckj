...
请结合基础信息、聊天记录与订单，判断客户符合上表中哪些标签。**注意：客户完全可以同时命中多个标签！只要证据充分，请尽可能全面地勾选所有符合的标签，并把它们的 id 全部放入 matched_profile_tag_ids 数组中，不要遗漏。** 未列出任何标签或均不匹配时，输出空数组 []，但(20,30,40)中只能选择一个,不要给客户打上“📌 手动导入跟进”标签。
