...
请根据提供的客户基础信息、最近聊天记录、订单历史记录以及近期联系任务（含申诉反馈），以专业的销售视角对该客户进行深度画像分析。
...
【近期联系任务与申诉反馈】
{{task_context}}
说明：每条任务含截止日、周期（日/周/月）、渠道（微信/电话）、状态、标题与执行建议；「申诉原因」为销售认为不应按该任务触达客户的反馈，须纳入 ai_profile 与 suggested_followup_date 的判断；「系统自动完成」表示聊天/语音等行为已触达，可佐证实际联系频率。

【可匹配的客户动态标签】
{{profile_tags_catalog}}
请结合基础信息、聊天记录、订单与联系任务，判断客户符合上表中哪些标签。**注意：客户完全可以同时命中多个标签！只要证据充分，请尽可能全面地勾选所有符合的标签，并把它们的 id 全部放入 matched_profile_tag_ids 数组中，不要遗漏。** 未列出任何标签或均不匹配时，输出空数组 []。
...
- ai_profile: 仅针对**客户本人**做销售视角客情分析：性格、沟通习惯、需求痛点、成交推进建议、适宜触达节奏（结合任务完成/申诉情况）等，不超过100字。**禁止**在 ai_profile 中写入当前业务/销售微信号的名称、昵称、别名或「销售微信备注」等；此类信息由系统在对话时从数据库单独注入，与本 JSON 输出无关。

- suggested_followup_date: 请根据客户的采购月份(purchase_months)、采购习惯（如每年固定月份下单）、聊天记录中的信息回复频率与活跃度、近期联系任务执行情况与申诉反馈进行综合分析，推断出最佳的下次跟进日期（格式：YYYY-MM-DD）。分析思路：
  1. 若客户有明确的采购月份（如每年 10 月采购），建议在采购前 1-2 个月跟进
  2. 若客户回复积极、有近期需求意向，建议在 1-2 周内跟进
  3. 若客户较冷淡或长期未回复，建议在 1 个月后跟进
  4. 若近期任务被申诉跳过（申诉原因说明不宜频繁联系、渠道不对等），应拉长跟进间隔并在 ai_profile 中体现
  5. 若昨日/近日任务已完成且客户已有效触达，避免 suggested_followup_date 落在过近的日期
  6. 若信息不足无法推断，留空
